...
public class ClaimPolicyProvider : IAuthorizationPolicyProvider
{
    ...
    public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
    {
        ...
        {
            var segments = policyName.Split('_', StringSplitOptions.RemoveEmptyEntries);
            var resource = segments[1];
            var claims = ulong.Parse(segments[2]);

            var policy = new AuthorizationPolicyBuilder();
            policy.AddRequirements(new ClaimRequirement(resource, claims));
